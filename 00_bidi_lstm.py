...
X_train, X_val, y_train, y_val = train_test_split(X_main, y_main, test_size=0.11111111, random_state=42,
                                                  stratify=y_main,
                                                  shuffle=True)
...
# model = Sequential([
#     Embedding(len(tokenizer.word_index), 256, name="embedding"),
#     Bidirectional(LSTM(64, return_sequences=True, dropout=0.2)),
#     GlobalMaxPool1D(),
#     Dense(20, activation="relu"),
#     Dropout(0.05),
...
# history = model.fit(train_seqs, y_train, epochs=EPOCH, validation_data=(val_seqs, y_val), verbose=1, batch_size=32,
#                     shuffle=True)
#
...
# group_counts = ["{0:0.0f}".format(value) for value in confu.flatten()]
# group_percentages = ["{0:.2%}".format(value) for value in confu.flatten() / np.sum(confu)]
# labels = [f'{group_names[i]} ({group_counts[i]}, {group_percentages[i]})' for i in range(len(group_names))]
# labels = np.asarray(labels).reshape(2, 2)
# sns.heatmap(confu, annot=labels, fmt="", cmap="Blues")
# plt.title('Figure 3-2 Confusion Matrix')
#
# plt.show()
